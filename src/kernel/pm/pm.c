...
*/
PUBLIC int do_get_process_info(pid_t pid_id, struct process_buf *buf) {
	struct process *p;
	for(p = FIRST_PROC; p <= LAST_PROC; p++)
		if(p->pid == pid_id) {
			buf->idProcesso = p->pid;
				for(int i=0; i<NAME_MAX; i++) {
					buf->nomeProcesso[i] = p->name[i];
				}
			buf->estado = p->state;
			buf->prioridade = p->priority;
			buf->quantum = p->counter;
			buf->tempoUsuario = p->utime;
			buf->tempoKernel = p->ktime;
			return 0;
		}
	return -1;
}
